...
export const addEquipment = async (
  ...
  res
    ...
    .json({
      error: false,
      message: "aquipment added successfully",
      data: newequipment,
    });
};
...
export const editEquipments = async (
  ...
  const updateEquipment = await Equiment.findByIdAndUpdate(id, equipmentData, {
    new: true,
  });
...
export const deleteEquipments = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  const { id } = req.params;
  const deletedeuipment = await Equiment.findByIdAndUpdate(
    id,
    { isDeleted: true },
    { new: true }
  );
  ...
  res
    ...
    .json({
      ...
      data: deletedeuipment,
    });
};

export const getAllequipmetRequests = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
